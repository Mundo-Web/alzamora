...
@section('css_importados')
    ...
    <style>
        ...
        .portadamobil{
             background-image: url({{asset('images/img/portadamovil.png')}})
        }
    </style>
...
@section('content')
    <main>
        ...
        <section class="flex overflow-hidden flex-col bg-white lg:bg-transparent -mt-[96px]" id="portada">
            <div class="flex relative flex-col justify-center w-full min-h-[750px] max-md:max-w-full pt-[96px] pb-14 lg:pb-0 px-[5%] xl:px-[10%]">
               
                    <img loading="lazy"

                    src="{{asset($general[0]->imagenportada)}}"
                    onerror="this.src='{{ asset('images/img/portada.png') }}';"
                    alt="" class="object-cover absolute inset-0 w-full lg:size-full h-[600px] object-center hidden sm:flex" />
                    <img loading="lazy"

                    src="{{asset('images/img/portadamovil.png')}}"

                    alt="" class="object-cover object-top absolute inset-0 w-full lg:size-full h-[500px] lg:h-full  flex sm:hidden" />
                
                


                <div class="flex flex-col lg:flex-row gap-12 xl:gap-28 items-end text-white z-10 sm:mt-20 lg:mt-0">
                    <div class="flex flex-col justify-end w-full lg:w-1/2 h-96 lg:h-auto">
                        <h2 class="text-4xl sm:text-5xl md:text-6xl uppercase font-PreloBold">
                           @if(isset($general[0]->tituloportada))
                                {{ $general[0]->tituloportada }}
                           @endif
                        </h2>
                    </div>
                    
                    <div class="flex flex-col w-full lg:w-1/2 relative">
                        <div class="bg-white bg-opacity-20 w-full h-full absolute backdrop-blur-sm"></div>
                        <form id="formContactos" class="flex z-10 flex-col px-0 lg:px-6 py-6 mt-0 ">
                            @csrf    
                            <h2 class="w-full text-2xl lg:text-3xl leading-7 uppercase font-PreloBold max-w-md text-[#1C1C1A] lg:text-white">
                                ¿Tienes alguna pregunta o necesitas más información?
                            </h2>
                            <div class="flex flex-col mt-6 w-full text-base">
                                <div class="flex flex-col w-full">
                                    <div class="mb-3">
                                        <label for="fullNameContacto" class="sr-only">Nombre completo</label>
                                        <input type="text" name="full_name" id="fullNameContacto"
                                            class="gap-2.5 w-full p-4 focus:ring-0 focus:border-x-0 focus:border-t-0 border-b-black focus:border-b-white lg:border-b-white 
                                            border-transparent  bg-transparent text-[#1C1C1A] lg:text-white placeholder:text-[#1C1C1A] lg:placeholder:text-white tracking-wide font-PreloMedium text-base"
                                            placeholder="Nombre completo" required />
                                    </div>
                                    <div class="mb-3">
                                        <label for="telefonoContacto" class="sr-only">Teléfono</label>
                                        <input type="tel" id="telefonoContacto" name="phone"
                                             class="gap-2.5 w-full p-4 focus:ring-0 focus:border-x-0 focus:border-t-0 border-b-black focus:border-b-white lg:border-b-white 
                                            border-transparent  bg-transparent text-[#1C1C1A] lg:text-white placeholder:text-[#1C1C1A] lg:placeholder:text-white tracking-wide font-PreloMedium text-base"
                                ...
                                <button type="submit"
                                    class="-mb-8 font-PreloSemibold tracking-wide px-6 py-4 mt-7 w-full leading-tight uppercase bg-stone-900 min-h-[44px] text-white max-md:px-5 max-md:max-w-full">
                                    ENVIAR MENSAJE
                                </button>
                            </div>
                        </form>
        ...
        @if ($benefit->isEmpty())
        @else
            <section  class="flex overflow-hidden flex-col bg-cover bg-no-repeat" style="background-image:url({{asset('images/img/nosotros.png')}})">
                <div id="about" class="w-full max-md:max-w-full">
                        ...
                        <article  class="flex flex-col w-[50%] max-md:w-full">
                            <div class="flex flex-col items-center justify-center text-white my-auto py-20 max-md:max-w-full">
                                <div class="flex flex-col my-auto min-w-[240px] max-w-xl mx-auto px-[5%]">
                                    <h1 class="text-4xl md:text-5xl leading-10 uppercase max-md:max-w-full font-PreloBold tracking-wide">
                                         @if(isset($general[0]->tituloestadisticas))
                                                {{$general[0]->tituloestadisticas }}
                                         @endif
                                    </h1>
                                    <div class="flex flex-col mt-10 w-full text-lg leading-6 max-md:max-w-full font-PreloMedium">
                                        <div class="max-md:max-w-full">
                                            @if(isset($general[0]->descripcionestadisticas))
                                                {!! $general[0]->descripcionestadisticas !!}
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex gap-10 flex-wrap mt-10 uppercase">
                                    @foreach ($benefit as $beneficio)
                                        <div class="flex flex-col max-w-[120px]">
                                            <p class="text-5xl leading-tight max-md:text-4xl font-PreloBold">{{ $beneficio->titulo }}</p>
                                            <p class="text-xs leading-tight font-PreloMedium">{{ $beneficio->descripcionshort }}</p>
                                        </div>
                                    @endforeach  
                                    </div>
                                    {{-- <a href="#about-us"
                                        class="gap-2.5 self-start px-6 py-3 mt-10 text-base leading-tight uppercase bg-neutral-100 text-stone-900 max-md:px-5 font-PreloBold"
                                        tabindex="0">
                                        Sobre nosotros
                                    </a> --}}
                                </div>
        ...
        @if ($posts->isEmpty())
        @else
            <section class="flex overflow-hidden flex-col lg:flex-row bg-white w-full px-[5%] lg:px-0 lg:pl-[5%] lg:gap-10 xl:gap-0 py-16 lg:py-20" id="servicios">
                
                <article class="flex flex-col w-full lg:w-[50%] ">
                            <div class="flex flex-col items-center justify-center text-black my-auto max-md:max-w-full">
                                <div class="flex flex-col my-auto w-full lg:max-w-xl mx-auto">
                                    <h1 class="text-4xl md:text-5xl leading-10 uppercase max-md:max-w-full font-PreloBold">
                                        @if(isset($general[0]->tituloservicios))
                                            {{$general[0]->tituloservicios }}
                                        @endif
                                    </h1>
                                    <div class="flex flex-col mt-10 w-full text-lg leading-6 max-md:max-w-full font-PreloMedium">
                                        <div class="max-md:max-w-full">
                                        @if(isset($general[0]->descripcionservicios))
                                            {!! $general[0]->descripcionservicios !!}
                                            @endif
                                        </div>
                                    </div>
                                    {{-- <div class="flex flex-col justify-start items-start mt-5">
                                        @foreach($posts as $post)
                                            <div class="flex flex-row gap-2 justify-start items-center mb-3">
                                                <a id="servicio-{{$post->id}}" class="uppercase text-lg font-PreloSemibold hover:border-b hover:border-black">{{$post->title}}
                                                </a>
                                                <img src="{{asset('images/svg/flecha.svg')}}"/>
                                            </div>
                                        @endforeach
                                    </div> --}}
                                    <div class="swiper-pagination-servicios mt-6"></div>
                                </div>
                            </div>
                </article>
                                    ...
                                    @foreach($posts as $servicios)  
                                        <div  class="swiper-slide servicio-{{$servicios->id}}">
                                            <div class="w-full sm:max-w-md flex flex-col gap-5 mx-auto">
                                                <img class="w-full object-cover object-center h-[500px] lg:h-[550px]" 
                                                src="{{ asset($servicios->url_image . $servicios->name_image) }}" 
                                                onerror="this.src='{{ asset('images/img/noimagenslider.jpg') }}';"
                                                />
                                                <div>
                                                    <h2 class="text-xl md:text-2xl uppercase font-PreloBold ">{{$servicios->title}}</h2>
                                                    <p class="text-base md:text-lg font-PreloMedium">{{$servicios->extract}}</p>
                                                </div>
                                                <a href="{{ route('detalleBlog', $servicios->id) }}" class="px-6 py-3 w-60 text-center text-base uppercase bg-[#1C1C1A] text-white max-md:px-5 font-PreloBold">MAS INFORMACIÓN</a>
                                            </div>
                                        </div>
                                    @endforeach     
                                    </div>
        ...
        @else
        <section class="flex overflow-hidden flex-col text-white bg-cover bg-no-repeat px-[5%] pt-[7%]" style="background-image:url({{asset('images/img/testimonios.png')}})">
            <article class="flex flex-col items-start w-full relative" >
                    <div>
                        <img class="object-contain object-bottom h-full w-min-[400px] pt-[350px] 3xs:pt-[300px] lg:pt-0" src="{{asset('images/img/chico.png')}}"  />
                    </div>
                    <div class="absolute top-0 xl:top-[10%] right-0 xl:right-[10%] w-full sm:max-w-2xl justify-center items-center text-center space-y-6">
                            <h2 class="uppercase text-4xl md:text-5xl font-PreloBold tracking-wide">
                                @if(isset($general[0]->titulotestimonios))
                                        {{$general[0]->titulotestimonios }}
                                @endif</h2>  
                                ...
                                <div class="swiper-wrapper">
                                 @foreach ($testimonie as $testimonios)
                                    <div class="swiper-slide">
                                        <div class="h-[110px]">
                                            <p class="font-PreloMedium text-lg line-clamp-4">{{ $testimonios->testimonie }}</p>    
                                        </div> 
                                        <h2 class="text-base font-PreloSemibold italic">{{ $testimonios->name }}</h2>  
                                    </div>
                                @endforeach 
                                </div>
                            </div>     
                    </div> 
                    <img class="absolute bottom-[40%] md:bottom-[30%] right-0 xl:right-[10%]" src="{{asset('images/img/comillas.png')}}"  />
            </article>
        </section>
        @endif
            ...
            <section class="hidden md:block">
                <div class="flex flex-col gap-8 w-11/12 mx-auto pb-20">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex justify-center items-center">
                            <a href="{{ route('blog', 0) }}"
                                class="text-white py-4 px-6 bg-[#1C1C1A] rounded-xl font-roboto font-semibold text-center">Ver
                                más publicaciones</a>
                        </div>
                        ...
                        @foreach ($posts->take(3) as $post)
                            <div class="flex flex-col gap-5">
                                <div class="flex flex-col gap-2">
                                    ...
                                    <h3 class="uppercase text-[#1C1C1A] font-roboto font-bold text-text12">
                ...
                <div class="flex flex-col gap-5 justify-center w-11/12 mx-auto z-[50] md:max-w-[620px] pt-10">
                    ...
                    <div class="flex justify-start items-center">
                        <a href="{{ route('descargables', 0) }}"
                            class="text-white font-roboto font-semibold text-text16 bg-[#1C1C1A] rounded-xl py-4 px-6">Descargar
                            Catálogos</a>
                    </div>
                </div>
        ...
        <section id="contacto">
            <div class="relative w-full px-[5%] py-16 lg:py-20">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 w-full">
                    <div class="flex flex-col items-center justify-center order-2 lg:order-1">
                            ...
                            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d975.3326187423952!2d-76.95748833045816!3d-12.08951358789787!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x9105c70a9644d30d%3A0x99e4ddbea03c4a83!2sAlzamora%20%26%20Asociados!5e0!3m2!1ses-419!2spe!4v1729709021917!5m2!1ses-419!2spe" 
                            width="100%" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>

                    </div>

                    <div class="flex flex-col gap-5 justify-center order-1 lg:order-2">
                        <div class="flex flex-col gap-1">
                            <h2 class="font-semibold font-PreloBold text-[#1C1C1A] text-3xl uppercase">
                                Datos de contacto
                            </h2>
                            <p class="font-PreloMedium text-[#1C1C1A] text-lg">Envíanos un mensaje y te responderemos a la brevedad.</p>
                        </div>
                                ...
                                <div>
                                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none"
                                        ...
                                        <path
                                            ...
                                            fill="#1C1C1A" class="h-7 w-7 fill-fillAzul md:fill-fillNaranja" />
                                    </svg>

                                </div>
                                <div class="flex flex-col gap-1">
                                    <p class="text-[#1C1C1A] font-PreloBold text-lg uppercase">Dirección
                                    </p>
                                    <p class="font-PreloMedium text-base text-[#1C1C1A] font-normal">
                        ...
                        <div class="flex gap-2">
                                ...
                                <div class="flex flex-col gap-1">
                                    <p class="text-[#1C1C1A] font-PreloBold text-lg uppercase">
                                        Números de
                                        Teléfono</p> 
                                    <p class="font-PreloMedium text-base text-[#1C1C1A] font-normal">
                                        @if(!empty($general[0]->office_phone) && !empty($general[0]->cellphone))
                                            {{ $general[0]->office_phone }} / {{ $general[0]->cellphone }}
                                        @elseif(!empty($general[0]->office_phone))
                                            {{ $general[0]->office_phone }}
                                        @elseif(!empty($general[0]->cellphone))
                                            {{ $general[0]->cellphone }}
                                        @endif
                                    </p>
                                </div>
                        ...
                        <div class="flex gap-2">
                                ...
                                <div class="flex flex-col gap-1">
                                    <p class="text-[#1C1C1A] font-PreloBold text-lg uppercase">Correo
                                        Electrónico</p>
                                    <p class="font-PreloMedium text-base text-[#1C1C1A] font-normal">
                                       {{ isset($general[0]->email) ? $general[0]->email : 'Ingresa un email' }}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <div class="flex flex-col gap-1">
                                <p class="text-[#1C1C1A] font-PreloBold text-lg uppercase ">Horario de
                                    Atención</p>
                                <p class="font-PreloMedium text-base text-[#1C1C1A] font-normal">
...
@section('scripts_importados')
    ...
    <script>
       function applyScrollStyles() {
            ...
            if (window.scrollY > 50) {
                ...
                botoncotizar.classList.remove('bg-white');
                botoncotizar.classList.add('bg-black');
                botoncotizar.classList.remove('text-black');
                botoncotizar.classList.add('text-white');
                logoImg.src = "{{ asset('images/svg/logoalzamorawhite.svg') }}";
            } else {
                header.classList.remove('bg-white');
                header.classList.add('bg-transparent');
                menu.classList.remove('text-black');
                menu.classList.add('text-white');
                botoncotizar.classList.remove('bg-black');
                botoncotizar.classList.add('bg-white');
                botoncotizar.classList.remove('text-white');
                botoncotizar.classList.add('text-black');
                logoImg.src = "{{ asset('images/svg/logoalzamora.svg') }}";
            }
        }
    ...
    // Agrega un evento a cada enlace con el id de "servicio-{id}"
    document.querySelectorAll('a[id^="servicio-"]').forEach(function (element) {
        element.addEventListener('click', function (event) {
            event.preventDefault(); // Prevenir comportamiento por defecto
            
            // Extraer el id del servicio
            const servicioId = event.currentTarget.id.split('-')[1];
            
            // Encontrar el índice de la diapositiva con la clase "servicio-{id}"
            const targetSlide = document.querySelector(`.swiper-slide.servicio-${servicioId}`);
            
            if (targetSlide) {
                const index = Array.from(targetSlide.parentNode.children).indexOf(targetSlide);
                
                // Cambia a la diapositiva correspondiente
                swiper.slideTo(index);
            }
        });
    });
